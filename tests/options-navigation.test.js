...
test('options shell keeps brand and sidebar fixed while the main settings area scrolls', () => {
  ...
  assert.match(optionsCss, /--options-brand-height:\s*124px/)
  assert.match(optionsCss, /--options-main-gutter:\s*clamp\(28px,\s*5vw,\s*88px\)/)
  assert.match(optionsCss, /--options-sidebar-gap:\s*28px/)
  ...
  assert.match(optionsCss, /\.options-header\s*\{[\s\S]*?position:\s*sticky[\s\S]*?top:\s*24px[\s\S]*?width:\s*100%[\s\S]*?min-height:\s*var\(--options-brand-height\)[\s\S]*?padding:\s*18px\s+16px\s+0/)
