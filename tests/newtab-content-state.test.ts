...
test('newtab dashboard glass layer only styles the iframe shell', () => {
  ...
  assert.match(newtabCss, /\.newtab-dashboard-surface\s*\{[\s\S]*?width:\s*min\(2200px,\s*100%\)/)
  assert.match(newtabCss, /\.newtab-dashboard-surface\s*\{[\s\S]*?height:\s*min\(1040px,\s*calc\(100vh - 64px\)\)/)
